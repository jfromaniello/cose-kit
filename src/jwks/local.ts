...
function getKtyFromAlg(alg: unknown) {
  switch (typeof alg === 'string' && alg.slice(0, 2)) {
    ...
    case 'Ed':
      return 'OKP'
    default:
      throw new COSENotSupported('Unsupported "alg" value for a JSON Web Key Set')
  }
}
...
/** @private */
export class COSELocalJWKSet<T extends KeyLike = KeyLike> {
  ...
  async getKey(signature: SignatureBase): Promise<T> {
    const { algName: alg, kid } = signature;
